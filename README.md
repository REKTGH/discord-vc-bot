...
- Run `/leaderboard`, `/leaderboard-here`, `/cancel`, `/log-here`, `/awards-here`, and `/help` to see the slash commands.
...
To turn this on, go to whichever channel you want the announcement in and run **`/awards-here`**. A few things worth knowing:
...
- Only server members with the **Manage Server** permission can run `/awards-here` (same reasoning as `/leaderboard-here` and `/log-here`).
