...
3. Install the project's dependencies:
   ...
   npm install
   ```
   This downloads the libraries the bot uses (discord.js, chrono-node, dotenv). It's normal for this to take a minute.
...
- Run `/leaderboard`, `/leaderboard-here`, `/cancel`, `/log-here`, and `/help` to see the slash commands.
...
`/leaderboard` and the live message from `/leaderboard-here` both render the same table, ranked latest to least late:

```
      Late     Avg Time Late     Cancels  No Show
1st   IamREKT  5 times  10 minutes late   3        1
```

- **Placement / User** (unlabeled columns) — rank, then display name. 1st = latest on average; long names get truncated to keep the table readable.
- **Late** — how many of their tracked joins were actually late (not early, not on time).
...
| `BOT_TIMEZONE` | `America/Los_Angeles` | Timezone used to understand "at 9pm" and to display times. [Full list of valid names](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones). |
...
- **The `/leaderboard-here` and `/log-here` channels reset the same way the leaderboard data does.** On a host without persistent storage (Render's free tier), a redeploy wipes which channel was set for each, same as it wipes the results themselves — you'll need to re-run whichever ones you use after a redeploy.
...
**`/leaderboard-here` or `/log-here` don't show up when I type `/`, but the other commands do.**
Both are restricted to members with the **Manage Server** permission by design (see [Live leaderboard channel](#live-leaderboard-channel) / [Voice-join log channel](#voice-join-log-channel)) — if that's not you, ask someone who has it, or have them adjust it in **Settings → Integrations**.
