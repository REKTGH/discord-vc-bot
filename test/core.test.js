...
ok('non-empty leaderboard renders a table with headers, placement, and username', () => {
  const now = new Date();
  ...
  assert.match(embed.data.description, /```/); // wrapped in a code block for monospace column alignment
  assert.match(embed.data.description, /Late\s+Avg Time Late\s+Cancels\s+No Show/); // headers, in order
  assert.match(embed.data.description, /🥇\s+Dave/); // placement (1st = medal) + username on the data row
});
ok('top 3 placements get medals, 4th place and beyond fall back to plain ordinals', () => {
  const now = new Date();
  ...
  ['Gold', 'Silver', 'Bronze', 'Fourth'].forEach((name, idx) => {
    ...
    recordResult({ guildId: 'gMedals', userId: name, username: name, targetTime: now, actualTime: new Date(now.getTime() + mins * 60000), diffSeconds: mins * 60, status: 'late', rawText: 'x' });
  });
  const embed = buildLeaderboardEmbed('gMedals');
  assert.match(embed.data.description, /🥇\s+Gold/);
  assert.match(embed.data.description, /🥈\s+Silver/);
  assert.match(embed.data.description, /🥉\s+Bronze/);
  assert.match(embed.data.description, /4th\s+Fourth/);
});
ok('tardy count, average lateness, cancels, and no-shows each land in their own column', () => {
  const now = new Date();
  recordResult({ guildId: 'gViewStats', userId: 'flip', username: 'Flip', targetTime: now, actualTime: new Date(now.getTime() + 10 * 60000), diffSeconds: 600, status: 'late', rawText: 'x' });
  recordResult({ guildId: 'gViewStats', userId: 'flip', username: 'Flip', targetTime: now, actualTime: null, diffSeconds: null, status: 'cancelled', rawText: 'nvm' });
  recordResult({ guildId: 'gViewStats', userId: 'flip', username: 'Flip', targetTime: now, actualTime: null, diffSeconds: null, status: 'no_show', rawText: 'x' });

  const embed = buildLeaderboardEmbed('gViewStats');
  // 1 late join (averaging 10 min late), 1 cancellation, 1 no-show
  assert.match(embed.data.description, /🥇\s+Flip\s+1 time\s+10 minutes late\s+1\s+1/);
});
ok('long usernames are truncated so one long name can\'t blow out the whole table width', () => {
  const now = new Date();
  const longName = 'ThisUsernameIsWayTooLongForATable';
  recordResult({ guildId: 'gViewLongName', userId: 'longy', username: longName, targetTime: now, actualTime: now, diffSeconds: 0, status: 'on_time', rawText: 'x' });
  const embed = buildLeaderboardEmbed('gViewLongName');
  assert.ok(!embed.data.description.includes(longName), 'expected the full long name to be truncated');
