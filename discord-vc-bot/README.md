...
**Changed your mind?** Reply with "nevermind" or "nvm" and the bot cancels your plan quietly (reacting with 🚫) — it won't post a no-show note for it later. If you instead send a new time ("nvm, joining at 10 instead"), that's treated as an update — the new time replaces the old one automatically, no need to cancel first.
