...
// A message that's JUST a number ("30", "5") - nothing else but maybe trailing
// punctuation - is common shorthand for "I'll be on in that many minutes",
// same meaning as "be on in 30". Deliberately narrow: it only fires when the
// ENTIRE message is the number, so a number mentioned inside an unrelated
// sentence ("we scored 30 points") still needs a real intent phrase like the
// rest of this file requires. Also bounded to a plausible near-term range so
// a year, price, or other random bare number typed alone isn't mistaken for
// a join plan.
...
function parseJoinTime(text, { timezone = 'America/Los_Angeles', referenceDate = new Date() } = {}) {
  ...
  const normalized = normalizeShorthand(effectiveText);
...
// for whatever calendar day `date` falls on in `timeZone`. Used by the
// monthly awards feature to decide which month a record belongs to, and to
// find "last month" relative to right now.
function localYearMonth(date, timeZone) {
...
// The UTC instant of local midnight on the 1st of `year`-`month` (1-indexed)
// in `timeZone`. Used to find a calendar month's [start, end) boundaries as
// plain millisecond numbers, so the rest of the monthly-awards code doesn't
// need to think about timezones at all past this point. Same guess-then-
// refine trick as buildLocalDate() above, so a month boundary that happens
// to land on a DST transition still resolves correctly.
function localMonthStartUTC(year, month, timeZone) {
  const approxOffset = getTimezoneOffsetMinutes(timeZone, new Date(Date.UTC(year, month - 1, 1)));
  let guessUTC = Date.UTC(year, month - 1, 1) - approxOffset * 60000;
  const refinedOffset = getTimezoneOffsetMinutes(timeZone, new Date(guessUTC));
  guessUTC = Date.UTC(year, month - 1, 1) - refinedOffset * 60000;
  return guessUTC;
}
