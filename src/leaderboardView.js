...
function buildLeaderboardEmbed(guildId, { live = false } = {}) {
  ...
  if (!rows.length) {
    ...
    const lines = rows.map((row, i) => {
      ...
      const cancelLabel = row.cancelCount > 0 ? `, 🚫 ${row.cancelCount} cancelled` : '';
      return `${medal} **${row.username}** — ${avgLabel} (${pct}% on-time, ${row.totalCount} tracked${noShowLabel}${cancelLabel})`;
    });
